...
class Motion:
	...
	def __init__( self ):
		...
		self.captureWidth = 1080		# Set this to a high resolution. The video file will be scaled down using "width" and "height" to reduce noise.
		self.captureHeight = 720		# Setting these to more than 1920x1440 seems to cause slowdown (records at a lower framerate)
		...
		self.videoReduction = 1		# For best results set this to 2. A video recorded at 1920x1440 will be scaled by half and saved at 960x720, reducing noise.
		...
		self.minimumTail = 1.0			# how long to keep testing for motion after last activity before commiting file

		self.framerate = 30			# Video file framerate.
		self.rotation = 0			# Rotates image (warning: cropping will occur!)
		self.filepath = "/home/pi/Desktop/Rajat_test/"	# Local file path for video files
		...
		self.usePreviewWindow = True		# Whether the preview window will be opened when running inside X.
