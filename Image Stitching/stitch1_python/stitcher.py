...
class Stitcher:
    ...
    def load_images(self,image_list):
        ...
        new_size = (self.images[0].shape[1]/4,self.images[0].shape[0]/4)
        images = map(lambda x: cv2.resize(x,new_size),self.images)

        return images

    def process_images(self):
        self.image_list_ = self.load_images(self.image_list_)
        self.patched_image_ = self.stitch_images(self.image_list_[0],self.image_list_[1])
        
        cv2.imwrite("final.jpg",self.patched_image_)
    ...
    def find_bounding_box(self,homography,h1,w1,h2,w2):     
      ...
      #remap the coordinates of the image boundaires into the stiched image space
